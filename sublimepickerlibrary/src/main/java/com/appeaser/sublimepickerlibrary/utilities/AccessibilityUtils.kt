...
object AccessibilityUtils {
	fun makeAnnouncement(view: View?, announcement: CharSequence) {
		if (view == null)
			return
		if (SUtils.isApi_16_OrHigher) {
			...
			val arc = AccessibilityEventCompat.asRecord(event)
			arc.setSource(view)
			event.className = view.javaClass.getName()
			event.packageName = view.context.packageName
			event.isEnabled = view.isEnabled
